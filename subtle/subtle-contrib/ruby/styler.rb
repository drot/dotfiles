...
if(2429 > teeny)
  puts ">>> ERROR: %s needs at least subtle `0.9.2429' (found: %s)" % [
    $0, Subtlext::VERSION
  ]
  exit
end
  ...
  module Contrib # {{{
    class Styler < Gtk::Window # {{{
      ...
      def color_button(table, row, col, name) # {{{
        ...
        button.signal_connect("color-set") do |button|
          begin
            # Assemble hex color
            hexcolor = "#"
            hexcolor += (button.color.red   >> 8).to_s(16)
            hexcolor += (button.color.green >> 8).to_s(16)
            hexcolor += (button.color.blue  >> 8).to_s(16)

            # Pad zeroes
            while(7 > hexcolor.size) do
              hexcolor << "0"
            end
            ...
            if(@all.include?(name))
              ...
              @colors.keys.select { |k, v|
                ...
                @buttons[name].set_color(Gdk::Color.parse(hexcolor))
              end
            else
              @colors[name] = hexcolor
            end
      ...
      def expose(cr) # {{{
        ...
        # Sublets
        draw_item(cr, "sublet", 530, 40, :sublets)
        draw_text(cr, "|", 576, 15, :separator)
        draw_item(cr, "sublet", 579, 40, :sublets)
      end # }}}
